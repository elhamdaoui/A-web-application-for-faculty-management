...
class Actualite {
    ...
    public function stocker() {
        if ($this->estPresAStocker()) {
            ...
            if ($this->getCinP() != null) {
                /* verifie que le professeur est le responsable du module */
                $resV = $bdd->prepare('select id_m from nom_module where id_m=:idm and id_f=:idf and cin_p=:cin');
                $resV->execute(array('idf' => $this->getIdF(), 'idm' => $this->getIdM(), 'cin' => $this->getCinP()));
                $donV = $resV->fetch();
                $resV->closeCursor();
                if (empty($donV)) {
                    echo '<script>alert("Erreur: professeur non responsable de ce module !!");</script>';
                    return FALSE;
                } else {
                    /* stocker l'actualite */
    ...
    public function afficherAvecMoteurImg($cheminMoteurImgAct) {
        ...
        <div class="conte">
            ...
            ?>
            <!-- cette paragraphe ne s'affiche à un etudiant-->
            <p class="infos"><mark id="mark">poster par <?php echo $infos; ?></mark><em id="acc_date"><?php echo $this->getDate(); ?></em></p>
            <p class="btn_actalite">&nbsp;<input type="text" disabled="" style="background-color: wheat;"/>
                <input type="hidden" value="<?php echo $this->getId(); ?>" name="id_acc_sup_mod"/>
                <input type="submit" value="modifier" id="modifier" name="modifier" onclick="gererClickBouttons('etes-vous sur de modifier?');"/>
                <input type="submit" value="supprimer" id="supprimer" name="supprimer" onclick="gererClickBouttons('etes-vous sur de supprimer?');"/>
            </p>
        </div>
        <?php
    }
     ...
     * @return array
     */
    public static function actualitesAllsAdmins() {
        global $bdd;
        $acts = array();
        $result = $bdd->query('select *from actualite where cin_ad is not null order by date_acc desc,id_acc desc');
        while ($ac = $result->fetch()) {
            $act = new Actualite();
            $act->remplir($ac['titre_acc'], $ac['contenu_acc'], $ac['date_acc'], $ac['image_acc'], $ac['cin_ad'], $ac['cin_p'], $ac['id_m'], $ac['id_f'], $ac['id_acc']);
            array_push($acts, $act);
     ...
     * @return type
     */
    public static function getActualitesIntersserEtudiant($cne) {
        global $bdd;
        ...
        if (!empty($idsmmods)) {
            $idsm = join($idsmmods, ',');
            $resActs = $bdd->prepare('select ac.titre_acc,ac.contenu_acc,ac.date_acc,ac.cin_ad,ac.cin_p,ac.id_m,ac.id_f,ac.id_acc,p.nom_p,p.prenom_p,md.nom_mod,m.nom_s
        from actualite ac,professeur p,nom_module md,modul m
        where ac.cin_p is not null 
        and ac.id_m in(' . $idsm . ') 
        and ac.cin_p=p.cin_p 
        and ac.id_f=(select e.id_f from etudiant e where e.cne_e=:cne and e.numins_e is not null) 
        and md.id_m=ac.id_m
        and md.id_f=ac.id_f
        and m.id_m=ac.id_m
        order by ac.date_acc desc');
            $resActs->execute(array('cne' => $cne));
            $acts = $resActs->fetchAll();
            $resMods->closeCursor();
            $resActs->closeCursor();
            return $acts;
        } else {
