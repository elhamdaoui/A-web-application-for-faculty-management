...
class Etudiant extends Personne {
    ...
    public function validerInscription() {
        ...
        if (isset($num_ins) and $num_ins != NULL) {
            ...
            try {
                ...
                return 'inscription a bien faite ';
            } catch (Exception $e) {
                return 'un probleme de stockage !!' . $e->getMessage();
            }
        }
    ...
    public function getReclamations() {
        global $bdd;
        $result = $bdd->prepare('select id_r,type_r,contenu_r,etat_r,date_r from  reclamation where cne_e=:cne order by date_r desc');
        $result->execute(array('cne' => $this->getCne()));
     ...
     * @param type $conten
     */
    public function envoyerReclammation($type, $conten) {
        global $bdd;
        $res = $bdd->prepare('insert into reclamation(type_r,contenu_r,date_r,cne_e) 
        values(:typ,:cont,:dat,:cne)');
        $res->execute(array('typ' => $type, 'cont' => $conten, 'dat' => date('Y-m-d'), 'cne' => $this->getCne()));
        $res->closeCursor();
    }
    ...
    /**
     * methode qui permette de retourner un tableau contient les inscriptions
     * du semestre passe au parametres
     *  
     * @global type $bdd
     * @param type $sem
     * @return boolean
     * @throws Exception
     */
    public function getInscriptionModulesSemestre($sem) {
        global $bdd;
        try {
            if (!$this->verifieChamp($sem)) {
                throw new Exception('Semestre non existe');
            }
            $resIns = $bdd->prepare('select ins.note_n,ins.note_r,ins.date_ins,ins.nb_ins,ins.etat_v,m.nom_s,md.id_m,md.nom_mod,m.id_eq
                from inscription ins,nom_module md,modul m
                where ins.cne_e=:cne and md.id_m=ins.id_m and md.id_m=m.id_m and m.nom_s=:sem and md.id_f=:idf');
            $resIns->execute(array('cne' => $this->getCne(), 'sem' => $sem, 'idf' => $this->getId_f()));
            $insSem = $resIns->fetchAll();
            $resIns->closeCursor();
            return $insSem;
        } catch (Exception $e) {
    ...
    /**
     * modifier l'email,adresse,nTel,password pour etudiant
     * @global type $bdd
     */
    public function modifENAP() {
        global $bdd;
        $res = $bdd->prepare('update etudiant set email_e=:email,ntel_e=:ntel,adresse_e=:adress,pwd_e=:pwd where cne_e=:cne');
        $res->execute(array('email' => $this->getEmail(), 'ntel' => $this->getNtel(), 'adress' => $this->getAdresse(), 'pwd' => $this->getPwd(), 'cne' => $this->getCne()));
        $res->closeCursor();
    }
     ...
     * @throws Exception
     */
    public function existePourConnecter($nom, $cin, $cne, $pwd) {
        try {
            global $bdd;
            if (empty($nom) || !isset($nom) || empty($cin) || !isset($cin) || empty($cne) || !isset($cne) || empty($pwd) || !isset($pwd)) {
                throw new Exception('des informations sont incorrectes');
            }
            $res = $bdd->prepare('select nom_e,prenom_e,cne_e,pwd_e,cin_e from etudiant 
                where cne_e=:cne and cin_e=:cin and pwd_e=:pwd and nom_e=:nom');
            $res->execute(array('cne' => $cne, 'cin' => $cin, 'pwd' => sha1($pwd), 'nom' => strtoupper($nom)));
            $don = $res->fetch();
            $res->closeCursor();
            if (empty($don)) {
                throw new Exception('Compte non existe');
            }
            return TRUE;
        } catch (Exception $e) {
            echo '<script>alert("' . $e->getMessage() . '");</script>';
